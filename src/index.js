...
const auth = (token) => new Promise((res, rej) => {
...
const services = {
  signup: (call, callback) => {
    ...
    if (email == undefined || password == undefined || username == undefined ||
        iban == undefined || email == '' || password == '' || iban == '' ||
        username == '')
      return callback({
    ...
    signupService(
        pool, grpc.status, email,
        crypto.createHash('sha256').update(password).digest('hex'), username,
        iban)
        .then((data) => {
          console.log(data);
  ...
  buy: (call, callback) => {
    auth(call.metadata.get('token'))  // get rate from exchange micro
        .then(({decoded}) => {
          ...
          return new exchangeMicroservice('exchange_microservice:9000')
  ...
  listTransactions: (call, callback) => {
    auth(call.metadata.get('token'))
        .then(
            ({decoded}) => listTransactionsService(
                pool, grpc.status, decoded.id, call.request))
